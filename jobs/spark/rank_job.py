...
def parser():
    result = argparse.ArgumentParser(
        description="OpenRec rank model training job"
    )
    result.add_argument("--date", required=True)
    result.add_argument("--revision", default="r001")
    result.add_argument("--scene", default="scene_0")
    result.add_argument(
        "--event-path", default="hdfs://namenode:8020/openrec/hive/event"
    )
    result.add_argument(
        "--item-path", default="hdfs://namenode:8020/openrec/hive/item"
    )
    result.add_argument(
        "--user-path", default="hdfs://namenode:8020/openrec/hive/user"
    )
    result.add_argument("--artifact-root", default="/models/releases")
    result.add_argument("--feature-selection", type=json.loads, default=None)
    ...
    result.add_argument(
        "--max-materialization-seconds", type=int, default=1800
    )
...
def _validate(args):
    ...
    if (
        args.max_history_rows < args.max_events
        or args.max_materialization_seconds < 1
    ):
...
def _freeze_feature_history(all_events, labelled_events):
    ...
    cutoff = labelled_events.agg(F.min("time").alias("cutoff")).first()[
        "cutoff"
    ]
    if cutoff is None:
...
def _user_pair_events(events, users, max_events):
    """
    Build balanced point-in-time U2U labels from active users in one label
    window.
    """
    user_ids = users.select(
        F.col("id").cast("string").alias("id")
    ).dropDuplicates()
    positive = (
        events.filter(F.col("type").isin("click", "buy", "collect"))
    ...
    negative_rank = Window.partitionBy("user_id").orderBy(
        F.xxhash64("user_id", "item_id")
    )
    negatives = (
        active_ids.alias("l")
        .crossJoin(active_ids.alias("r"))
    ...
    positives = (
        pairs.orderBy(F.xxhash64("user_id", "item_id"))
        .limit(per_class_limit)
        .withColumn("type", F.lit("click"))
    )
    negatives = (
        negatives.orderBy(F.xxhash64("user_id", "item_id"))
        .limit(per_class_limit)
    ...
    return (
        ...
        .withColumn(
            "trace_id", F.concat_ws("-", F.lit("u2u"), "user_id", "item_id")
        )
...
def run(args, spark=None):
    ...
    spark = (
        spark
        or SparkSession.builder.appName("openrec-rank-train")
        .enableHiveSupport()
        .getOrCreate()
    )
    business_day = datetime.strptime(args.date, "%Y-%m-%d").replace(
        tzinfo=timezone.utc
    )
    label_days = (
        args.user_label_window_days if args.target_type == "user" else 1
    )
    label_from = int(
        (business_day - timedelta(days=label_days - 1)).timestamp()
    )
    label_until = int((business_day + timedelta(days=1)).timestamp())
    # This immutable boundary makes reruns reproducible: mutations arriving
    # after the business
    # window closed cannot add, update, or retract labels retrospectively.
    label_observation_cutoff = label_until
    all_events = read_events_as_of(
        spark,
        date=args.date,
    ...
    label_window = all_events.filter(
        (
            F.lit(True)
            if args.scene == "global"
            else F.col("scene") == args.scene
        )
        & (F.col("time") >= label_from)
        & (F.col("time") < label_until)
    )
    events = (
        label_window.filter(F.col("type").isin("click", "expose"))
        .orderBy(F.desc("time"))
        .limit(args.max_events)
    )
    input_label_count = events.count()

    # The provisional snapshot is used only while constructing labels. Full
    # entity histories and
    # behaviour up to the latest label are exported below and resolved for each
    # individual label.
    provisional_cutoff = events.agg(F.min("time").alias("cutoff")).first()[
        "cutoff"
    ]
    ...
    items = read_items(
        ...
        F.lit(True) if args.scene == "global" else F.col("scene") == args.scene
    )
    users = read_users(
        spark,
        date=args.date,
    ...
    if feature_cutoff_time is None or feature_until_time is None:
        raise ValueError(
            "rank training data has no labelled events "
            "after target construction"
        )
    feature_events = read_event_history(
        spark,
        date=args.date,
        path=args.event_path,
        until_time=feature_until_time,
    )
    items = read_item_history(
        spark,
        date=args.date,
        path=args.item_path,
        until_time=feature_until_time,
    ...
    if history_rows > args.max_history_rows:
        raise ValueError(
            "PIT history rows %d exceed gate %d"
            % (history_rows, args.max_history_rows)
        )
    started = time.monotonic()
    sample_events, sample_users, sample_items = (
        materialize_point_in_time_samples_spark(
    ...
    if not materialized_count:
        raise ValueError(
            "rank training data is empty after point-in-time joins"
        )
    if elapsed > args.max_materialization_seconds:
        raise ValueError(
            "PIT materialization took %.1fs, exceeding gate %ds"
            % (elapsed, args.max_materialization_seconds)
        )
    version = "%s-%s" % (args.date.replace("-", ""), args.revision)
    dataset_dir = (
        Path(args.artifact_root).parent
        / "training"
        / args.target_type
        / args.scene
        / version
    ...
    try:
        # Spark's default filesystem is HDFS. The trainer reads the driver's
        # shared model volume, which is not mounted on Spark executors.
        # Stream rows instead of collecting the entire dataset into memory.
        for frame, filename in (
            (sample_events, "events.jsonl"),
            (sample_users, "sample_users.jsonl"),
            (sample_items, "sample_items.jsonl"),
        ):
